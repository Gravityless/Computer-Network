...
def test_hub():
    ...
    # add my testcase: a broadcast frame should be sent out all ports except ingress
    testpkt = new_packet(
        "40:00:00:00:00:02",
        "ff:ff:ff:ff:ff:ff",
        "192.168.0.1",
    ...
    s.expect(
        ...
        ("my testcase: a broadcast frame should arrive on eth0")
    )
    s.expect(
        PacketOutputEvent("eth1", testpkt, "eth2", testpkt, display=Ethernet),
        ("my testcase: a broadcast frame should be sent out other ports")
    )
